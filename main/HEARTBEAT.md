...
2. Check for tasks where Assignee=MaoKu and Status=Todo
   - If found: pick highest priority, then:
     ...
     b. **Can execute independently** → Pick up (post 🚀 in Topic, update Status=In Progress) → Execute (progress in Topic)
...
- Create a Task Board record for each violation: Name="[Audit] 修复: {issue}", Assignee={violator}, Status=Todo, Priority=P1
